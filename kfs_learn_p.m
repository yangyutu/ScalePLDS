...
iter=1;dif=1;
Pt=zeros(d,d,T); % similar to a tensor, try in the command window zeros(2,2,3)
Ptt_1=zeros(d,d,T-1); % T here is for the time in a time series
fistiter = 25; 
...
while (iter<miter)&&(dif>tol)
    ...
    %   estimate A,  need L1 norm regularization
...
%    fistL = 2*eigs(d,1); % shaojie made a mistake here
%    yuguang change above  two lines to the following two lines
     eigs = eig(fistA);
     fistL = 2*max(eigs)^2; % from fista paper
