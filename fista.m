...
% yuguang comment out the following line, because i do not think it is
% useful
%eigvals = eig(AtA);
%lambda = lambda * exp(mean(log(abs(eigvals))));

[~,d] = size(AtA);
% yuguang: the following x and y are initial guess for the algorithm
...
% yuguang change the above line to the following, because the 2 should not exist
    grad = mat_vec_prod(AtA,p,y) - b;
